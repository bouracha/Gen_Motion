...
class VDVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        # Top Down
        self.reconstructions_mu, self.zs, self.KLs = self.decoder(encoder_activations)
    ...
    def generate(self, z, distribution='gaussian', latent_resolution=0, z_prev_level=0):
        ...
        if z_prev_level==0 and z!=None:
            self.zs["0"] = z
            self.decoder.residuals_dict["0"] = self.decoder.reshape_z0_linearly(self.zs["0"])

        for i in range(z_prev_level, len(self.decoder.feature_hierachies)-2):
        ...
        decoder_output_final = self.decoder.decoder_block_final(self.decoder.residuals_dict[str(len(self.decoder.feature_hierachies)-2)])
