...
def random_hyperparameters():
    ...
    return n_z, num_decoder_stage, alpha, lambda_


for i in range(opt.num_trials):
    (n_z, num_decoder_stage, lr, lambda_) = random_hyperparameters()
    command = 'python3 main.py --lr ' + str(lr) + ' --lambda ' + str(lambda_) + ' --n_z ' + str(
        ...
        opt.epoch) + ' --input_n 10 --output 10 --dct_n 20 --data_dir h3.6m/dataset/ --out_of_distribution walking'
    os.system(command)
