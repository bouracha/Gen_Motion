...
for num_occlusions in range(0, 97, 3):
    ...
    for i in range(1):
        ...
        z_val_degraded = mapper.transform(X_val_degraded)
    ...
    file_name = "imputed_noise_2"
    if first_loop:
        df.to_csv('umap/'+str(file_name)+'.csv', header=head, index=False)
        first_loop=False
    else:
        with open('umap/'+str(file_name)+'.csv', 'a') as f:
